...
제품명 아래에는 현재 분석 목적을 한 문장으로 설명하고, 모바일에서는 현재 단계를 `현재 / 전체` 형식으로 함께 표시한다.
...
- 상단: `분석 사례를 선택하세요`
- 기본 경로는 `대표 사례`로 두고 `직접 입력`, `JSON 업로드`는 대체 입력 방식으로 제공
- 대표 사례 선택 시: 정상·경계·고위험·결측 사례를 한 줄 또는 반응형 2열로 표시
- 각 사례에는 이름, 한 줄 설명, 예상되는 입력 특징만 표시
- `분석 설정` 안에서 `전체 분석`, `규칙 기반 분석만`을 선택
- 하단 왼쪽: `입력 초기화`
- 하단 오른쪽 주 버튼: `입력 확인`
...
- 식별자·원본 코드·생성 메타데이터는 세부 정보나 저장 JSON에 배치하고 기본 화면을 복잡하게 만들지 않음
...
- 첫 영역: 통합 결과 제목과 위험 신호 수준
- 두 번째 영역: `분만 전 → 초기 CTG → ML 분석` 3단계 흐름
- 세 번째 영역: 단계별 역할, 상태 배지와 최대 3개의 핵심 근거
- 입력 완전성과 누락 항목은 통합 결과 가까이 표시
- 모델 내부 분석값은 기본 화면에서 강조하지 않고 `모델 분석 상세`에 배치
- 마지막 영역: 결과와 기술 세부 정보
- 하단 행동 순서: 주 행동 `결과 JSON 저장`, 보조 행동 `새 사례 분석`, `입력 수정`
...
- 등급은 색상, 문구와 형태가 있는 상태 배지를 함께 사용한다.
- 모델 분석값은 상세 영역에서 소수점 한 자리 백분율로 표시하고 실제 발생 확률로 표현하지 않는다.
...
- 모델이 없거나 호환되지 않을 때 규칙 결과는 유지되고 모델 영역은 `이 단계는 현재 분석할 수 없습니다`로 안내한다.
...
- 합성 데이터·비임상용 제한은 UI가 아닌 제출 자료와 결과 계약에 기록한다.
...
- 대표 사례를 선택해 결과를 저장하는 필수 흐름이 3분 시연에 맞게 구성된다.
...
다른 팀원의 무지원 데모와 비임상용 문구 교차 검토는 구현 완료 이후 사용자와 팀이 수행하는 인수 절차다. 이는 Codex의 구현 목표나 완료 판정을 보류하는 조건에 포함하지 않으며, 결과는 별도 확인표에 기록한다.
...
| 문구 | 자연스러운 한국어, 금지 표현, 제출 자료와 UI의 역할 분리 |
...
팀원의 무지원 데모, 발표 리허설, 주관적 문구 검토와 최종 팀 승인은 사용자 측 인수 활동이다. 구현 완료 이후 수행하되 Codex가 달성해야 하는 기술 완료 기준에는 포함하지 않는다.
